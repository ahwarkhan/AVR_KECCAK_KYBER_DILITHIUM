...
static uint8_t g_pk[PQCLEAN_KYBER768_CLEAN_CRYPTO_PUBLICKEYBYTES];
static uint8_t g_sk[PQCLEAN_KYBER768_CLEAN_CRYPTO_SECRETKEYBYTES];
static uint8_t g_ct[PQCLEAN_KYBER768_CLEAN_CRYPTO_CIPHERTEXTBYTES];
static uint8_t g_ss[PQCLEAN_KYBER768_CLEAN_CRYPTO_BYTES];
static uint8_t g_ss2[PQCLEAN_KYBER768_CLEAN_CRYPTO_BYTES];
...
int main(void)
{
	...
	uart_puts("=== Kyber768 REF Benchmark (AVR/simavr) ===\n");
	...
	timer_reset();
	PQCLEAN_KYBER768_CLEAN_crypto_kem_keypair(g_pk, g_sk);
	...
	/* --- Encaps ---------------------------------------------------- */
	timer_reset();
	PQCLEAN_KYBER768_CLEAN_crypto_kem_enc(g_ct, g_ss, g_pk);
	...
	/* --- Decaps ---------------------------------------------------- */
	timer_reset();
	PQCLEAN_KYBER768_CLEAN_crypto_kem_dec(g_ss2, g_ct, g_sk);
	...
	/* --- Correctness ----------------------------------------------- */
	uint8_t ok = 1;
	for (uint8_t i = 0; i < PQCLEAN_KYBER768_CLEAN_CRYPTO_BYTES; i++)
		if (g_ss[i] != g_ss2[i])
		{
			ok = 0;
			break;
