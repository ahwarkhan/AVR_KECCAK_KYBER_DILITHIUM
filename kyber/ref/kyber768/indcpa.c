...
void PQCLEAN_KYBER768_CLEAN_indcpa_keypair(uint8_t pk[KYBER_INDCPA_PUBLICKEYBYTES],
    ...
    gen_a(a, publicseed); // 2,587,135, rej : 9*3283 = 29,547, keccak : 2,587,135 - 29,547

    for (i = 0; i < KYBER_K; i++) {
        PQCLEAN_KYBER768_CLEAN_poly_getnoise_eta1(&skpv.vec[i], noiseseed, nonce++); //118,693*3
    }
    for (i = 0; i < KYBER_K; i++) {
        PQCLEAN_KYBER768_CLEAN_poly_getnoise_eta1(&e.vec[i], noiseseed, nonce++); //118,693*3
    } 
    ...
    for (i = 0; i < KYBER_K; i++) {
        PQCLEAN_KYBER768_CLEAN_polyvec_basemul_acc_montgomery(&pkpv.vec[i], &a[i], &skpv); //181,527*3
        PQCLEAN_KYBER768_CLEAN_poly_tomont(&pkpv.vec[i]); //19,735*3
    } // 603,797
...
void PQCLEAN_KYBER768_CLEAN_indcpa_enc(uint8_t c[KYBER_INDCPA_BYTES],
    ...
    gen_at(at, seed); // 2,587,069 -> 2557588

    for (i = 0; i < KYBER_K; i++) {
        PQCLEAN_KYBER768_CLEAN_poly_getnoise_eta1(sp.vec + i, coins, nonce++); //118,695 -> 96841
    }
    for (i = 0; i < KYBER_K; i++) {
        PQCLEAN_KYBER768_CLEAN_poly_getnoise_eta2(ep.vec + i, coins, nonce++); //118,695 -> 96841
    }
    ...
    PQCLEAN_KYBER768_CLEAN_poly_invntt_tomont(&v); //175,837
...
void PQCLEAN_KYBER768_CLEAN_indcpa_dec(uint8_t m[KYBER_INDCPA_MSGBYTES],
    ...
    PQCLEAN_KYBER768_CLEAN_polyvec_ntt(&b); //306755
    PQCLEAN_KYBER768_CLEAN_polyvec_basemul_acc_montgomery(&mp, &skpv, &b); //181529
    PQCLEAN_KYBER768_CLEAN_poly_invntt_tomont(&mp); //175837
